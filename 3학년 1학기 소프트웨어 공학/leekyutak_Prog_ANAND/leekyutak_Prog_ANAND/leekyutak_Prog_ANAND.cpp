...
int KB, Byte, A_sector, sector, R_sector, R_block_count;
			...
			for (int u = 0; u < A_sector; u++) {
				if (CINT3 == lsn[u]) {                     //입력받은 값의 블록번호와 일치하는 LBN을 찾고
					if ((strcmp(A_DATA[CINT3].data, "O") != 0) && (strcmp(A_DATA[CINT3].data, "X") != 0)) {      //그 들어온값의 섹터가 아무런 값도 적혀있지 않다면
						...
						printf("%d번 블록 오프셋 %d번에 저장되었습니다. %s\n\n", CINT3_block, CINT3_offset, A_DATA[CINT3].data);
						WriteCount++;
					}
					else {
						//printf("\n\n%s\n\n", A_DATA[CINT3].data);
						strcpy(A_DATA[CINT3].data, "X");
						if (CINT3_block < R_block_count) {//비어있는 이유??????????
						...
						for (int i = CINT3_block; i <= R_block_count; i++) {
							...
							if ((strcmp(A_NAND[K].data, "O") != 0) && (strcmp(A_NAND[K].data, "X") != 0)) {
								...
								printf("1.대체 블록%d번 %d섹터에 저장 완료 %s\n\n", i, CINT3_offset, A_NAND[K].data);
								WriteCount++;
								break;
							}
							if (i == R_block_count) {
								...
								printf("2.대체 블록%d번 %d섹터에 저장 완료 %s\n\n", i + 1, CINT3_offset, A_NAND[K + 64].data);
								WriteCount++;
								break;
							}
						}
