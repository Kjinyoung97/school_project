...
int KB, Byte, A_sector, R_sector, sector, R_sector_count;
			...
			for (int u = 0; u < A_sector; u++) {
				if (CINT3 == lsn[u]) {							//입력받은 값의 블록번호와 일치하는 LBN을 찾고
					if ((strcmp(A_DATA[CINT3].data, "O") != 0) && (strcmp(A_DATA[CINT3].data, "X") != 0)) {		//그 들어온값의 섹터가 아무런 값도 적혀있지 않다면
						...
						A_DATA[CINT3].spae = CINT3_offset;
						WriteCount++;
					}
					else {
						//printf("\n\n%s\n\n", A_DATA[CINT3].data);
						strcpy(A_DATA[CINT3].data, "X");  //이미 내용이 있다면 해당데이터를 비유효 데이터로 처리
						for (int i = 0; i < R_sector; i++) {
							//printf("%d\t %d\t %d\n", K, R_block_count, CINT3_block);
							if ((strcmp(A_NAND[i].data, "O") != 0) && (strcmp(A_NAND[i].data, "X") != 0)) {
								strcpy(A_NAND[i].data, "O");
								A_NAND[i].spae = CINT3_offset;
							...
							if (i = R_sector) {
								...
								R_sector_count++;
								strcpy(A_NAND[i].data, "O");
								printf("2.대체 블록%d번 %d섹터에 저장 완료 %s\n\n", i + 1, CINT3_offset, A_NAND[i + 64].data);
								WriteCount++;
								break;
							}
						}
...
void margin() {
	...
	while (Y <= R_sector_count) {
		for (int Q = 0; Q < 64; Q++) {
			int T = Q;
			int Z = Q;
			while (T <= 13000) {//이부분이 실행되지 않음 
