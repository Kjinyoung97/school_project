...
void Table_print(struct MEMORY *MEMORY);
			...
			for (int u = 0; u < sector2; u++) {
				...
				if (CINT3 == lsn[u]) {
					...
					if ((strcmp(MEMORY[u].data, check1) == 0) || (strcmp(MEMORY[u].data, check2) == 0)) {
						...
						for (int k = sector2; k < sector; k++) {
							if ((strcmp(MEMORY[k].data, check1) != 0) && (strcmp(MEMORY[k].data, check2) != 0)) {
								printf("여분 블록 PSN %d에 저장하였음.\n", k);
